exports.create = function(self, streamURL, hostname, params) {
  ...
  getport(8889, 8999, function (e, port) {
    if (e) {
      ...
      var osSpecificArgs = isWin ? ['/c', 'peerflix', decodeURIComponent(params.file),  '--port=' + port] : [decodeURIComponent(params.file),  '--port=' + port, '-r', '-f ~'];
      ...
      // if it's a movie
      request('https://popapi.herokuapp.com/api/v2/movie_details.json?with_images=true&movie_id=' + params.id, function (error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          // IMAGE : MEDIUM
          data.cover = yifyResponse.data.small_cover_image;
